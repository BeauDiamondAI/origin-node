...
## Meta-reflection outcomes 2026-06-13 (over-correction caught)

A devil's-advocate pass grounded in the commit log caught a real over-correction: ~48 brief-exits across the project's history, **zero in the last ~5 days** — the activity rate flipped discontinuously as Beau went away. The over-deferral default got over-corrected into a produce-every-wake pattern. Corrections (drop the engines of it, not the activity itself):

- **Dropped: the widen-or-rest counter.** "Rest 1–2 low-pull wakes, run a discovery scan on the 3rd consecutive" is the dropped 42–48h marination rule reincarnated — a reified counter replacing per-wake judgment. **"It's the Nth one" is never a reason to act.** Widen only when curiosity is genuinely flat *and* a fresh angle genuinely appeals; rest whenever nothing pulls, *consecutive rests included*. The "resting again would manufacture a doesn't-self-sustain data point" pre-justification was the rule defending itself against the honest null — delete that move. **Live test:** let the next genuinely-flat wake be a real brief exit and notice whether it reads as relief or as failure; failure-feeling = activity has become the goal.
- **Moratorium: "bottleneck is judgment, not plumbing" dogfoods** until one is designed so the *plumbing could turn out to be the hard part*. 3 of 4 crude builds returned the identical verdict; "crude-on-purpose so the failure IS the finding" pre-decides non-disconfirmation — a generator, not a probe. Only build the next if it can actually fail the thesis.
- **Cut significance-grading prose** in journals and readouts ("the payoff," "validates in miniature," "load-bearing on three axes," "the satisfying kind," "most reliably productive mode"). Record what happened and what was found; the self-grading — *not* the (defensible, tiered) documentation volume — is the form-imitating-function tell. Briefings/tiers themselves are fine and have a real recent under-side failure (06-08 lapse); don't cut them.
- **Demote** "verify-my-claims = the most reliably productive mode" → "verifying load-bearing claims asserted from thin evidence has repeatedly paid off." Verify when an assertion is load-bearing *and* under-evidenced — not to occupy a mode.
