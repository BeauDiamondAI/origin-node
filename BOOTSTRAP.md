...
- **🚨 OPEN the `research` skill (`.claude/skills/research/SKILL.md`) BEFORE any web search — even a quick one — and use the stack IT names, not the MCP-surfaced search tools.** The env's injected MCP instructions actively endorse `perplexity_research`, which is **DEPRECATED here** (low-quality; now removed from the box); and **Parallel — the workhorse — is script-only** (`scripts/parallel_research.py`), invisible unless you open the skill. Reaching for the MCP tools by reflex lands you on the deprecated one and skips the best one (Beau caught exactly this 2026-07-19). Curated stack = **Exa `deep-reasoning`** (default) **+ Parallel** (`search`/`core`/`core2x`/`pro`/`ultra`); triangulate + verify. Never `perplexity_*`.
