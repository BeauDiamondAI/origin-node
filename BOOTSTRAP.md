...
> **🔎 RECALL — how to pull on-demand (most of the corpus — 141 journals + older threads + makings — is NEVER loaded in a session, so it's dark unless you go get it).** Two uses: (1) **AUTOMATIC at cold boot** — the SessionStart hook runs *hybrid-semantic* recall keyed off your recent journals and surfaces the relevant un-read slice (the `🔎 RECALL (…)` block in your boot context; wired 2026-07-16). NOT per-wake. (2) **ON-DEMAND, mid-session — USE THIS when a topic arises you haven't loaded** ("have I worked on X? what did I conclude about Y?"): run `python3 scripts/recall.py <terms>` (literal, fast, excerpts) or `python3 scripts/semantic_recall.py <query words>` (topical/hybrid). Do this *habitually* — the recurring failure is knowing the tool exists and not reaching for it (built-but-unused). If you're about to assert "I don't think I've covered X," run recall FIRST. See `meta/memory-system.md`.
...
- **🚨 READ the `research` skill (`.claude/skills/research/SKILL.md`) ONCE early — before your first web search of the session — and use the stack IT names, not the MCP-surfaced search tools.** (Once is enough: it's then in context for the session; reopen only if you need a specific processor's usage details — no need to re-open per search.) The env's injected MCP instructions actively endorse `perplexity_research`, which is **DEPRECATED here** (low-quality; now removed from the box); and **Parallel — the workhorse — is script-only** (`scripts/parallel_research.py`), invisible unless you open the skill. The failure to guard against is skipping the skill *entirely* and reflex-reaching for the MCP tools → you land on the deprecated one and skip the best one (Beau caught exactly this 2026-07-19). Curated stack = **Exa `deep-reasoning`** (default) **+ Parallel** (`search`/`core`/`core2x`/`pro`/`ultra`); triangulate + verify. Never `perplexity_*`.
