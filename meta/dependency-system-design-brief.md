...
- **Build status:** Layers 0 (refs.py) built; **Layer 3 empirically validated by a manual dogfood (2026-06-13, `journal/2026-06-13-0000-layer3-judgment-dogfood.md`)** — run directly (the LLM *is* the judgment layer) over refs.py's 27 structural candidates for one changed file. Result: **27 → ~5 (≈80% noise reduction)**, the bulk demoted at high confidence, validating both halves of this brief in miniature — (a) the layer dramatically works (over-flagging → precise list), and (b) the irreducible residue is real and sits exactly where predicted (the ~5 borderline *restatement-vs-pointer* cases needing confidence-gating, with residual silent-failure risk). A crisp operational signature fell out as a candidate prompt: **live = the file *restates/builds-on* the claim; dead = it merely *names/records/points to* it.** Layers 1–2 (claim-extraction, embeddings) and a *scripted* Layer 3 remain unbuilt — natural next steps only if they pull.
