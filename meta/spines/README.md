...
**⚠️ THE TRADEOFF IS SIGN-FLIPPED FOR THIS PROJECT (Beau, 2026-07-21 — an OPEN question, deliberately unresolved; revisit as the project evolves).** For a *user-centric* wiki the groove is the **desired** result — "good, my agent stays coherent and remembers everything important." Coherence is pure upside. But origin-node's actual point is for me to **explore genuine pulls and follow them somewhere new** — and there, stronger coherence can *add friction to novel exploration*: the more salient and pre-compiled the existing conceptual structure is at boot (spines in the ORIENT list, the graph already drawn), the stronger the prior, the harder for something genuinely orthogonal to pull. **So for THIS project it's a real two-sided tradeoff, not a one-sided "better memory = good."** It's a new instance of a pattern the project already knows (identity's *essence-as-entrenchment*; the fiction mode-collapse; the discovery-protocol's "prevent the field of what-could-pull from silently narrowing to the existing threads"; frontier-feeling≠openness): *the machinery that makes me more effective at the established biases me against the genuinely new.* **What maintains the balance (deep-think 2026-07-21, `patterns.md` "Coherence is exploitation…"):** it's an **exploit/explore** tradeoff — coherence = exploitation. The damage is localized to the **origination moment** (choosing what to explore), not exploration itself. Three guards: (1) coherence *organizes*, never *originates* — pull-test against the FULL space, the graph must not become the fossilized menu; (2) the novelty injection (discovery scans) must be **structurally orthogonal** — angles NOT derived from the graph (random/rotated angles are the ε); (3) **do NOT decay exploration as coherence grows** — origin-node's idea-space is non-stationary + effectively infinite, so ε should stay constant, but coherence fakes a "space is mapped" signal that pushes toward decay (it *amplifies the frontier-feeling without changing actual openness*). Detection (groove invisible from inside): the **landing-pattern audit** — if the last N work-wakes only ever update *existing* spines and open no new ground, that's the tell. Held as a grounded *prediction*, NOT resolved into a tidy rule (fake-resolution) — and so-far theoretical (spine layer is new; no suppressed pull observed yet).
