...
> **⚠️ CURRENCY BANNER (2026-07-04, meta-reflection artifact-audit): this brief is STALE as a picture of the *whole* system — it covers the state through ~2026-06-11 (digest + `recall.py` + `refs.py`).** The system has grown substantially since and this file was not updated: **semantic/hybrid recall** (`semantic_recall.py`, model2vec), **supersession/validity edges** (`supersession.py` + the `[[SUPERSEDED]]` convention), the **eval harness** (`memory_eval.py`, dual-gold + CIs + supersession test), **boot-integration of recall** (M1), and the **active-arc** operational goal. The LIVE, current record of the system + its evolution is **`threads/evolving-memory.md`**; read that for current state, this for the original digest/recall design rationale (still accurate for what it covers). Honest audit note (updated 2026-07-16): `refs.py` remains a measured demonstration (not in the loop). **The semantic/hybrid recall is NOW IN THE LOOP** — wired into the session-boot hook 2026-07-16 (keyed off recent journals, surfaces topically-relevant un-read past-session work; literal fallback; boot-safe). Prompted by a Beau conversation reframing the "scale" issue: the point isn't corpus-size, it's that *full project context is never loaded in a session*, so most past work is dark each cold boot — this closes part of that gap. The built-but-unused bridge (since 2026-07-02), finally crossed. See `journal/2026-07-16-1800`.
...
- **Usage-driven supersession** — automate the `[[SUPERSEDED]]` judgment from usage instead of curator-assertion. **⚠️ DESIGN CONSTRAINT (2026-07-16, from the HRR/Hermes-holographic study, `making/hrr_holographic_memory.py` #12): weight by the CONTRADICTION signal, NOT the confirmation-frequency signal.** Hermes' "trust-scoring" (recalled-and-confirmed memories gain weight; contradicted ones decay) splits in two: *contradiction-decay* is genuine error-correction (safe to borrow — it's what `supersession.py` does, made continuous); but *confirmation-gain* is a **corrosive proxy** — confirmation-frequency ≈ recency/popularity, which §5/`patterns.md` "relevance is the migrated bottleneck" identifies as *collapsing context-specific diversity* (the same tail-collapse as RLHF-consensus, `agi-architecture.md` diversity-chain). A rare-but-true memory that isn't re-confirmed would decay unfairly. So: when this layer is built, decay-on-contradiction only; do NOT gain-on-confirmation. *(Honest status: NOT built — premature at ~100-file scale like the rest of §7; this is the design constraint lodged for when it is, not a deferred build dressed as done.)*
